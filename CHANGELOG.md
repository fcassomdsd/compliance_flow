...
- **`/inspectionPlan` answered `200` with an empty body, and a bad `provider` or `siteVisit` wedged the flow.** The plan itself was generated correctly all along — the defects were the response and the error path. The flow generates the plan and then reuses the same `msg` to set the inspection status to `Planned` (`msg.entity`/`entityId`/`updateData`), which destroyed the generation result before the response node ran, so no caller could tell what had been generated or where it went. The result is now kept aside and the endpoint answers with it: `generatedFile.name`/`path`/`version`/`downloadURL`, plus `inspectionId` and `inspectionStatus`. Four lookups also indexed `msg.payload.list[0]` unguarded (`save provider data`, `add contact info`, `get contact name and services`; and `save inspected provider data` in the report tab), so an unknown id produced `TypeError: Cannot read properties of undefined (reading 'serviceProviderId')` instead of a named error — and because the flow's `debug` tap responds before the catch runs, the failed request had already answered `200` with an empty body, so the error path logged `ERR_HTTP_HEADERS_SENT` and "Message exceeded maximum number of catches" and left the flow mis-answering until Node-RED was restarted. Each lookup now fails with a named envelope (`No inspected provider found for id: …`, `No site visit found for code: …`) and the flow stays healthy: verified live that a bad `provider` returns `400 {"success":false,"error":"No inspected provider found for id: nope"}` and the next good call still returns the plan. Documented on the way: `/inspectionPlan` takes the **inspected-provider** id while `/inspectionReport` takes the **service-provider** id — the same `provider` parameter with two meanings, which is why the report answers `No inspected provider found for id: demo-iprov-ans` until it is given `demo-prov-ans`.

### Changed

- **`.env.example`'s `API_KEY` now ships a placeholder value instead of empty, so the gateway guard is on by default.** Previously left blank ("dev mode"), which meant every clone/demo instance ran with an unauthenticated gateway unless someone opted in. The shipped value is a public placeholder committed to this repo — `demo-only-CHANGE-BEFORE-ANY-PUBLIC-DEPLOYMENT` — and must be rotated (`openssl rand -hex 32`) together with the matching `NODE_RED_API_KEY` (`compliance_web`) and `IMPORT_API_KEY` (`compliance_import`) before any deployment reachable by anyone untrusted; `compliance_checklist` stores one key and sends it to both services already. README's auth table and Security Notes section updated to match. No code changed — `httpNodeMiddleware` already did the right thing whenever `API_KEY` was set.

- **The smoke harness and the error-envelope audit send `X-API-Key` when one is configured.** Both are run by `atrocore-docker/scripts/demo-quickstart.sh`, so with the gateway guard on they reported `401` for every check — the suite would have failed on a correctly hardened deployment. They now read `API_KEY` (or `NODE_RED_API_KEY`, the name `compliance_web` uses) and send the header; with no key configured they behave exactly as before. Verified against a live stack with the guard enabled: bare request `401`, correct key `200`, wrong key `401`, and both harnesses green (smoke 15/15, envelope audit 5/5).

- **`scripts/smoke-flows.mjs` now defaults to the committed demo dataset's ids, not one instance's.** Its defaults were records that existed only on the long-lived development instance (`osvaldo.delgadillo`, `a01m1xst8faebz93nas6xdht053`, `a01m1xss7xge4z9gygvcmj3mb64`, `MDPP`), so on a fresh clone — precisely where `atrocore-docker/scripts/demo-quickstart.sh` runs it — four checks failed: `GET /inspector/:externalId` and `GET /inspection/:inspectionId` answered `404`, `GET /providers?siteVisit=` returned an empty array, and `GET /checklist` timed out on a 20 s abort. They now default to `demo.inspector1`, `demo-sv-01`, `demo-insp-ans-01`, `demo-iprov-ans`, specialty `ATS` and location `ZZZZ` — the stable ids in `atrocore-docker/sql/seed-demo-dataset.sql`. Every override (`INSPECTOR_EXTERNAL_ID`, `SITE_VISIT_ID`, `INSPECTION_ID`, `INSPECTED_PROVIDER_ID`, `SPECIALTY_CODE`, `LOCATION_CODE`) keeps working unchanged. Verified against a freshly provisioned instance: **15 passed, 0 failed**, 10 skipped (destructive).
- **README points at the whole-platform demo quickstart, which lives in `atrocore-docker`.** §7 of `atrocore-docker/docs/COMPLIANCE_INTEGRATION_RUNBOOK.md` — executable as `atrocore-docker/scripts/demo-quickstart.sh` — is the single documented clean-clone-to-demonstrable sequence, and §8 holds the failure-isolation cues for a misbehaving flow, so this README now links to both instead of implying the steps are repo-local. No endpoint, node or flow changed; the CI-checked endpoint manifest is unaffected.
...
- **The `Auth flows` tab had two identical catch nodes, so every error was answered twice.** Node-RED delivers an error to *every* matching `catch` node, and both were unscoped (`scope: null`) with their own `error envelope` → `http response` chain. The first response succeeded and the second tripped `ERR_HTTP_HEADERS_SENT`; that error was itself caught, and the loop left the flow unable to answer — the first request returned an envelope and every later request hung (observed as ~680 error lines a minute and a `SIGABRT` exit during the `/inspection` fix). One unscoped catch node remains. Verified with AtroCore stopped: two consecutive error calls each answer `{"success":false,…}` and the tab logs **zero** duplicate-response errors, where before the second call hung. `scripts/validate-flows.mjs` now fails CI when a flow has more than one unscoped catch node (node count 403 → 400).
